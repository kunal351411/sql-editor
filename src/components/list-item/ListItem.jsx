...
import './ListItem.css';
import { savedTablesState, savedQueriesState, queryHistoryState, selectedQueryState } from '../../state/atoms';
import { capitalizeString } from '../../utils/helpers/helpers';

const ListItem = ({type, text, id, link = ''}) => {
  let savedState;
  if(type === 'table')
    savedState = savedTablesState;

  else if(type === 'query')
    savedState = savedQueriesState;

  else
    savedState = queryHistoryState;
  ...
  return (
    <div className='container'>
      <div className='list-text' onClick={(type !== 'table' && selectedQuery !== text) ? selectQuery : null}>{type === 'table' ? (<a href={link} target='_blank' rel="noreferrer" >{text}</a>) : text}</div>
